function Invoice(param) {
    ...
    this.getdataurl = "invoices/getdata?team=" + $('#Teams').val() + '&customer='
                        + CookieManager.GetCookie('customer') + param + '&isNeedFilter=true';
};

$(function () {
    var param = "";
    var MAX_EMPLOYEE_LENGTH = 20;
    ...
    GridInitializer.InitGrid(
        [
            ...
            { name: 'Id', index: 'Id', hidden: false, sortable: false, width: 120 },
            { name: 'New', index: 'New', hidden: true },
            { name: 'Employee', index: 'Employee', sortable: false, width: 180 },
            { name: 'CarInfo', index: 'CarInfo', sortable: true, width: 300 },
            { name: 'TotalAmount', index: 'TotalAmount' },
            { name: 'PaidSum', index: 'PaidSum', sortable: false },
            { name: 'InvoiceStatus', index: 'InvoiceStatus', sortable: true, width: 110 },
    ...
    $('#StartSearch').click(function () {
        ...
        if (count == 0) {
            jAlert("Search did not find any Invoices", "No results found...");
        } else {
