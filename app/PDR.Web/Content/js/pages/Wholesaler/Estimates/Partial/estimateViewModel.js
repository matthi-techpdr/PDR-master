function EstimateViewModel() {
    ...
    self.selectBox = $('select.select-employee');
//    self.retailCustomer = new CustomerRetailInfo(self);
    ...
    self.inspection.hourlyRate(parseFloat(window.globals.currentCustomer.HourlyRate));
    self.inspection.discount(window.globals.currentCustomer.Discount);
    self.inspection.laborRate(window.globals.currentCustomer.LaborRate);
    viewModel.hasEstimateSignature(window.globals.currentCustomer.EstimateSignature ? 'True' : 'False');
...
//        self.inspection.discount(window.globals.currentCustomer.Discount);
//        self.inspection.laborRate(window.globals.currentCustomer.LaborRate);
  //      self.inspection.workByThemselve(window.globals.currentCustomer.WorkByThemselve);
//        viewModel.hasEstimateSignature(window.globals.currentCustomer.EstimateSignature ? 'True' : 'False');
...
//                        viewModel.hasEstimateSignature(data.HasEstimateSignature ? 'True' : 'False');
    ...
    self.wholesale.dataBindChange();
    //self.dgdsgdfsgdfsgdf();
    self.updateData();
...
//        var customerId = self.wholesale.wholesaleId.val();
//        var text = $('#Employee_Id :selected').text();
//        viewModel.customerInfo(text.length == 0 ? '' : ' - ' + text);
