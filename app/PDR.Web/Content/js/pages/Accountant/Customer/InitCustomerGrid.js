function Customer() {
    ...
    this.getdataurl = "customers/getdata?team=" + $('#team').val() + '&state=' + $('#UsaStates').val();
    this.getcurrenturl = "customers/getcustomer";
    this.editurl = "customers/editcustomer";
    this.createurl = "customers/createcustomer";
    this.suspendurl = "customers/suspendcustomer";
    this.reactivateurl = "customers/reactivatecustomer";
    this.name = 'customer';
};
...
//    $('#team').selectBox("value", 0);
    ...
    function saveJsonModel() {
        var model = {
            ...
            Email2: $("#customerForm #Email2").val(),
            Email3: $("#customerForm #Email3").val(),
            Email4: $("#customerForm #Email4").val(),
            Password: $("#customerForm #Password").val(),
            Discount: $("#customerForm #Discount").val(),
            ...
            MatricesIds: $("#customerForm #MatricesList").val(),
            Comment: $("#customerForm #Comment").val(),

            Insurance: $("#customerForm #Insurance").is(':checked'),
            EstimateSignature: $("#customerForm #EstimateSignature").is(':checked'),
            OrderSignature: $("#customerForm #OrderSignature").is(':checked'),
            WorkByThemselve: $("#customerForm #WorkByThemselve").is(':checked'),
            CanCreateEstimates: $("#customerForm  #CanCreateEstimates").is(':checked')
        };
        var id = $('#customerForm #Id').val();
        var form = $('#customerForm');
        var teamsList = $('#customerForm #TeamsList');
        var matricesList = $('#customerForm #MatricesList');
        if ((form.valid() && teamsList.val() != null && matricesList.val() != null)) {
            var url = id == "null" ? customer.createurl : customer.editurl;
            Helpers.SendJsonModel(customer.editform, url, customer.grid, model);
        }
        else if (teamsList.val() == null || matricesList.val() == null) {
            var el = teamsList.val() == null ? teamsList.next() : matricesList.next();
            if (teamsList.val() == null && matricesList.val() == null) {
                el = $('.ui-multiselect');
            }
