function Invoice(param) {
    ...
    this.getdataurl = "getdataforaccountant?archived=true&discarded=true&team=" + $('#Teams').val() + '&customer=' +
...
$(function () {
    ...
    GridInitializer.InitGrid(
        ...
        Helpers.InitDateRangeTextboxes('from', 'to', function () { Helpers.UpdateInvoiceGrid($('#discardedInvoicesGrid'), $('#from'), $('#to'), $('#Customers'), $('#Teams'), $('#Statuses'), 'getdataforaccountant?archived=false&discarded=true'); });

    $('#Statuses').change(function () {
        Helpers.UpdateInvoiceGrid($('#discardedInvoicesGrid'), $('#from'), $('#to'), $('#Customers'), $('#Teams'), $(this), 'getdataforaccountant?', '&archived=false&discarded=true');
    });

    $('#Customers').change(function () {
        CookieManager.SetCookie("customer", $(this).val());
        Helpers.UpdateInvoiceGrid($('#discardedInvoicesGrid'), $('#from'), $('#to'), $(this), $('#Teams'), $('#Statuses'), 'getdataforaccountant?', '&archived=false&discarded=true');
    });

    $('#Teams').change(function () {
        CookieManager.SetCookie("team", $(this).val());
        Helpers.UpdateInvoiceGrid($('#discardedInvoicesGrid'), $('#from'), $('#to'), $('#Customers'), $(this), $('#Statuses'), 'getdataforaccountant?', '&archived=false&discarded=true');
    });
    ...
    $('#StartSearch').click(function () {
        ...
        $('#discardedInvoicesGrid').setGridParam({
            url: 'getdataforaccountant?archived=true&discarded=true' + '&customer=' + $('#Customers').val() + "&team=" + $('#Teams').val() +
