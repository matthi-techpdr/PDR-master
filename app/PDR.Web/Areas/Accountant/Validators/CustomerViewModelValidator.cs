...
namespace PDR.Web.Areas.Accountant.Validators
{
    public class CustomerViewModelValidator : AbstractValidator<CustomerViewModel>
    {
        public CustomerViewModelValidator()
        {
            ...
            RuleFor(x => x.Zip).NotEmpty().WithMessage("Please specify zip").Length(1, 10);
            ...
            RuleFor(x => x.Discount).NotEmpty().Digits().WithMessage("Enter only digits").Length(0, 2);
        ...
        private static bool EmailBeUnique(CustomerViewModel model, string email)
        {
