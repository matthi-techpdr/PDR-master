...
namespace PDR.Web.Areas.Admin.Controllers
{
    public class RepairOrdersController : Common.Controllers.RepairOrdersController
    {
        ...
        public ActionResult AssignMoreTechnicians(long? ids)
        {
            ...
            var admins = this.userStorage.Get().Company.Employees.Where(x => x.Role == UserRoles.Admin && x.Status == Statuses.Active).Select(x => (TeamEmployee)x).Distinct().ToList();

            var model = GetRiOperations(repairOrder);

            var teamEmps = customerTeams.SelectMany(x => x.Employees
